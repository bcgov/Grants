...
To update the database use this command from the Web project folder (replace `Grants.ApplicantPortal.API` with your project's name):

```powershell
dotnet ef database update -c AppDbContext -p ../Grants.ApplicantPortal.API.Infrastructure/Grants.ApplicantPortal.API.Infrastructure.csproj -s Grants.ApplicantPortal.API.Web.csproj
```
