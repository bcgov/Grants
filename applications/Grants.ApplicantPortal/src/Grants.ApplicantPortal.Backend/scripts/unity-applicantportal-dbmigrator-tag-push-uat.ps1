# Script to build, tag and push Unity Applicant Portal DB Migrator image for uat
...
$OC_TARGET_PROJECT = "d18498-test"
$LOCAL_IMAGE_NAME = "applicant-portal-dbmigrator"
$IMAGESTREAM_NAME = "uat-applicantportal-dbmigrator"
...
oc delete jobs uat-applicantportal-dbmigrator -n $OC_TARGET_PROJECT --ignore-not-found=true
oc process unity-applicantportal-dbmigrator-job -n $OC_TARGET_PROJECT -p APPLICATION_GROUP=uat-unity-applicant-portal -p DATABASE_SERVICE_NAME=uat-unity-applicant-portal -p APPLICATION_NAME=uat-applicantportal-dbmigrator -p IMAGEPULL_NAMESPACE=$OC_TARGET_PROJECT -p IMAGESTREAM_NAME=$IMAGESTREAM_NAME -p IMAGESTREAM_TAG=$ImageTag | oc create -f -
oc wait jobs/uat-applicantportal-dbmigrator --for condition=complete --timeout=120s -n $OC_TARGET_PROJECT